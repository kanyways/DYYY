...
static BOOL DYYYTypeEncodingAllowed(const char *actualEncoding, const char *const *allowedEncodings, NSUInteger allowedEncodingCount) {
...
static NSString *DYYYRuntimeHookRegistryKey(DYYYRuntimeHookRequest request) {
    return [NSString stringWithFormat:@"%s|%@|%@", class_getName(request.targetClass) ?: "<missing>", request.classMethod ? @"+" : @"-", NSStringFromSelector(request.selector)];
}
...
NSString *DYYYRuntimeHookInstallStatusName(DYYYRuntimeHookInstallStatus status) {
    switch (status) {
        case DYYYRuntimeHookInstallStatusInstalled:
            return @"installed";
        case DYYYRuntimeHookInstallStatusAlreadyInstalled:
            return @"already-installed";
        case DYYYRuntimeHookInstallStatusTargetClassMissing:
            return @"class-missing";
        case DYYYRuntimeHookInstallStatusTargetMethodMissing:
            return @"method-missing";
        case DYYYRuntimeHookInstallStatusTypeEncodingMismatch:
            return @"encoding-mismatch";
        case DYYYRuntimeHookInstallStatusReplacementMissing:
            return @"replacement-missing";
        case DYYYRuntimeHookInstallStatusReplacementSelfReference:
            return @"replacement-self-reference";
        case DYYYRuntimeHookInstallStatusConflict:
            return @"conflict";
    }
    return @"unknown";
}
