...
static BOOL DYYYHighFPSEnabled(void) {
    return [DYYYUtils fastBoolForKey:@"DYYYEnableHighFPS"];
}
...
static id DYYYCallSharedIfPossible(Class cls) {
    ...
    static const char *kSharedSels[] = {
        "sharedInstance", "sharedManager", "p_sharedInstance", "defaultManager", "manager",
    };
...
static CFTypeRef DYYYCFBundleGetValueForInfoDictionaryKey(CFBundleRef bundle, CFStringRef key) {
    ...
    if (DYYYHighFPSEnabled() && key && CFStringCompare(key, CFSTR("CADisableMinimumFrameDurationOnPhone"), 0) == kCFCompareEqualTo) {
...
static void DYYYInvokeProMotionFPSBooster(void) {
    ...
    static const char *kBoostSels[] = {
        "boostFPSForScrolling", "boostFPSForAppearance", "boostFPSForVCAppearance", "SetHighRefreshRate", "setHighRefreshRate",
    };
...
static void DYYYHighFPSEnterThrottle(void) {
    ...
    NSProcessInfo *info = [NSProcessInfo processInfo];
    NSLog(@"[DYYY][RuntimeHook][HighFPS] 自动降档 thermal=%ld lowPower=%d", (long)info.thermalState, info.isLowPowerModeEnabled);
...
static void DYYYInstallLoadObserversIfNeeded(void) {
    ...
    gDYYYThermalObserver = [center addObserverForName:NSProcessInfoThermalStateDidChangeNotification
                                               object:nil
                                                queue:[NSOperationQueue mainQueue]
                                           usingBlock:^(__unused NSNotification *note) {
                                             DYYYHighFPSReevaluateLoad();
                                           }];
    gDYYYPowerObserver = [center addObserverForName:NSProcessInfoPowerStateDidChangeNotification
                                             object:nil
                                              queue:[NSOperationQueue mainQueue]
                                         usingBlock:^(__unused NSNotification *note) {
                                           DYYYHighFPSReevaluateLoad();
...
static void DYYYApplyProMotionUnlock(BOOL enabled) {
    ...
    NSLog(@"[DYYY][RuntimeHook][HighFPS] ProMotion门闩 enabled=%d mutate=%d write=%d runtime=%@ diskOriginal=%d maxFPS=%ld", enabled, mutated, written, runtimeValue, gDYYYProMotionDiskKeyValue,
