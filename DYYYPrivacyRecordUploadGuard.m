...
+ (BOOL)objectContainsBlockedUpload:(nullable id)object enabledMask:(DYYYPrivacyRecordUploadMask)enabledMask depth:(NSUInteger)depth visited:(NSMutableSet<NSValue *> *)visited;
+ (BOOL)collectionContainsBlockedUpload:(nullable id)collection enabledMask:(DYYYPrivacyRecordUploadMask)enabledMask depth:(NSUInteger)depth visited:(NSMutableSet<NSValue *> *)visited;
...
+ (BOOL)shouldBlockRequestObject:(id)requestObject {
    ...
    return [self objectContainsBlockedUpload:requestObject enabledMask:enabledMask depth:0 visited:visited];
}

+ (BOOL)shouldBlockAwemeViewRecordURLString:(NSString *)URLString {
    return [self isAwemeViewRecordUploadDisabled] && [self uploadKindForURLString:URLString] == DYYYPrivacyRecordUploadKindAwemeView;
}
...
+ (BOOL)markTaskForBlockingIfNeeded:(id)task requestObject:(id)requestObject {
    ...
    objc_setAssociatedObject(task, kDYYYPrivacyRecordUploadBlockedTaskKey, @YES, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    return YES;
}
...
+ (void)invokeCancelledCompletionIfPossible:(id)completion {
    ...
    NSError *error = [NSError errorWithDomain:NSURLErrorDomain code:NSURLErrorCancelled userInfo:@{NSLocalizedDescriptionKey : @"cancelled by DYYY privacy guard"}];
...
+ (DYYYPrivacyRecordUploadKind)uploadKindForURLString:(NSString *)URLString {
    ...
    BOOL mayBeProfileVisit = [URLString containsString:kDYYYProfileVisitRecordUploadMarker] || [URLString containsString:kDYYYProfileVisitRecordUploadPath];
    BOOL mayBeAwemeView = [URLString containsString:kDYYYAwemeViewRecordUploadMarker] || [URLString containsString:kDYYYAwemeViewRecordUploadPath];
...
+ (DYYYPrivacyRecordUploadKind)uploadKindForNormalizedPath:(NSString *)path {
    ...
    if ([path isEqualToString:kDYYYAwemeViewRecordUploadPath] || [self normalizedPath:path matchesMarker:kDYYYAwemeViewRecordUploadMarker]) {
        return DYYYPrivacyRecordUploadKindAwemeView;
    }
    if ([path isEqualToString:kDYYYProfileVisitRecordUploadPath] || [self normalizedPath:path matchesMarker:kDYYYProfileVisitRecordUploadMarker]) {
        return DYYYPrivacyRecordUploadKindProfileVisit;
    }
    return DYYYPrivacyRecordUploadKindNone;
}
...
+ (id)valueForSelector:(SEL)selector fromObject:(id)object {
    ...
    while (*unqualifiedReturnType == 'r' || *unqualifiedReturnType == 'n' || *unqualifiedReturnType == 'N' || *unqualifiedReturnType == 'o' || *unqualifiedReturnType == 'O' ||
           *unqualifiedReturnType == 'R' || *unqualifiedReturnType == 'V') {
...
+ (BOOL)objectContainsBlockedUpload:(id)object enabledMask:(DYYYPrivacyRecordUploadMask)enabledMask depth:(NSUInteger)depth visited:(NSMutableSet<NSValue *> *)visited {
    if (!object || depth > 8) {
        return NO;
    }
    ...
    if ([object isKindOfClass:NSURLRequest.class]) {
        DYYYPrivacyRecordUploadKind kind = [self uploadKindForURLString:((NSURLRequest *)object).URL.absoluteString];
        return [self shouldBlockUploadKind:kind enabledMask:enabledMask];
    }
    ...
    if ([object isKindOfClass:NSDictionary.class] || [object isKindOfClass:NSArray.class] || [object isKindOfClass:NSSet.class]) {
        return [self collectionContainsBlockedUpload:object enabledMask:enabledMask depth:depth visited:visited];
    }
    ...
    for (NSUInteger index = 0; index < sizeof(selectors) / sizeof(selectors[0]); index++) {
        ...
        if ([self objectContainsBlockedUpload:value enabledMask:enabledMask depth:depth + 1 visited:visited]) {
            return YES;
        }
    }
    return NO;
...
+ (BOOL)collectionContainsBlockedUpload:(id)collection enabledMask:(DYYYPrivacyRecordUploadMask)enabledMask depth:(NSUInteger)depth visited:(NSMutableSet<NSValue *> *)visited {
