...
static BOOL DYYYFPSOverlayShouldShow(void) {
    return [DYYYUtils fastBoolForKey:@"DYYYEnableHighFPS"] && [DYYYUtils fastBoolForKey:@"DYYYShowFPSOverlay"];
}
...
void DYYYStartFPSOverlay(void) {
    ...
    gDYYYFPSOverlayBecomeActiveObserver = [center addObserverForName:UIApplicationDidBecomeActiveNotification
                                                              object:nil
                                                               queue:[NSOperationQueue mainQueue]
                                                          usingBlock:^(__unused NSNotification *note) {
                                                            DYYYApplyFPSOverlaySettingChange();
                                                          }];
    gDYYYFPSOverlayResignActiveObserver = [center addObserverForName:UIApplicationWillResignActiveNotification
                                                              object:nil
                                                               queue:[NSOperationQueue mainQueue]
                                                          usingBlock:^(__unused NSNotification *note) {
                                                            [gDYYYFPSOverlayView stopSampling];
                                                          }];
    gDYYYFPSOverlayOrientationObserver = [center addObserverForName:UIApplicationDidChangeStatusBarOrientationNotification
                                                             object:nil
                                                              queue:[NSOperationQueue mainQueue]
                                                         usingBlock:^(__unused NSNotification *note) {
                                                           if (DYYYFPSOverlayShouldShow()) {
