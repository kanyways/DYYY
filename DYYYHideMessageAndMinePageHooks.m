...
static BOOL DYYYInstallInstanceBOOLHook(Class cls, SEL selector, IMP replacement, DYYYBoolIMP *originalSlot, BOOL required, const char *logLabel) {
    ...
    if (!existing) {
        if (required) {
            NSLog(@"[DYYY][RuntimeHook][HideMessageAndMinePage] %@ 缺少 %@：%@", logLabel, NSStringFromSelector(selector), NSStringFromClass(cls));
        }
        return NO;
    }
    ...
    if (!methodDefinedOnClass) {
        ...
        if (!class_addMethod(cls, selector, replacement, typeEncoding)) {
            if (required) {
                NSLog(@"[DYYY][RuntimeHook][HideMessageAndMinePage] %@ class_addMethod 失败：%@", logLabel, NSStringFromClass(cls));
            }
            return NO;
        }
    ...
    if (!previous || previous == replacement) {
        NSLog(@"[DYYY][RuntimeHook][HideMessageAndMinePage] %@ 无法保存原 IMP：%@", logLabel, NSStringFromClass(cls));
        return NO;
    }
...
static BOOL DYYYInstallClassBOOLHook(Class cls, SEL selector, IMP replacement, DYYYBoolContextIMP *originalSlot, BOOL required, const char *logLabel) {
    if (!cls || !replacement || !originalSlot) {
        return NO;
    }
    ...
    if (!existing) {
        if (required) {
            NSLog(@"[DYYY][RuntimeHook][HideMessageAndMinePage] %@ 缺少类方法 %@：%@", logLabel, NSStringFromSelector(selector), NSStringFromClass(cls));
        }
        return NO;
    }
    ...
    IMP previous = method_setImplementation(existing, replacement);
    if (!previous || previous == replacement) {
        NSLog(@"[DYYY][RuntimeHook][HideMessageAndMinePage] %@ 无法保存原类方法 IMP：%@", logLabel, NSStringFromClass(cls));
        return NO;
    }
...
static BOOL DYYYInstallInstanceSizeHook(Class cls, SEL selector, IMP replacement, DYYYSizeIMP *originalSlot, BOOL required, const char *logLabel) {
    if (!cls || !replacement || !originalSlot) {
        return NO;
    }
...
static BOOL DYYYInstallInstanceVoidUpdateUIHook(Class cls, SEL selector, IMP replacement, DYYYVoidUpdateUIIMP *originalSlot, BOOL required, const char *logLabel) {
    if (!cls || !replacement || !originalSlot) {
        return NO;
    }
...
static BOOL DYYYInstallInstanceIMPHook(Class cls, SEL selector, IMP replacement, IMP *originalSlot, BOOL required, const char *logLabel) {
    if (!cls || !replacement || !originalSlot) {
        return NO;
    }
...
static BOOL DYYYInstallLayoutSubviewsHook(Class cls, DYYYVoidLayoutIMP replacement, DYYYVoidLayoutIMP *originalSlot, BOOL required, const char *logLabel) {
    if (!cls || !replacement || !originalSlot) {
        return NO;
    }
    ...
    if (!existing) {
        if (required) {
            NSLog(@"[DYYY][RuntimeHook][HideMessageAndMinePage] %@ 缺少 layoutSubviews：%@", logLabel, NSStringFromClass(cls));
        }
        return NO;
    }
...
void DYYYStartHideMessageAndMinePageHooks(void) {
    ...
    if (resourceSlotClass &&
        DYYYInstallInstanceBOOLHook(resourceSlotClass, @selector(canShowInNaviBar), (IMP)DYYYMessageTabResourceSlotCanShowInNaviBar, &gOrigMessageTabResourceSlotCanShow, NO, "MessageTabStarMall")) {
        installed += 1;
    }

    Class resourceSlotV2Class = objc_getClass("AWEIMMessageTabNavBarResourceSlotComponentV2");
    if (resourceSlotV2Class && DYYYInstallInstanceBOOLHook(resourceSlotV2Class, @selector(canShowInNaviBar), (IMP)DYYYMessageTabResourceSlotV2CanShowInNaviBar, &gOrigMessageTabResourceSlotV2CanShow,
                                                           YES, "MessageTabStarMall")) {
        installed += 1;
    }

    Class storyPublishClass = objc_getClass("AWEProfileAvatarStoryPublishController");
    if (storyPublishClass) {
        if (DYYYInstallInstanceBOOLHook(storyPublishClass, @selector(canShow), (IMP)DYYYProfileStoryPublishCanShow, &gOrigProfileStoryPublishCanShow, YES, "MineAvatarPlus")) {
            installed += 1;
        }
        if (DYYYInstallInstanceVoidUpdateUIHook(storyPublishClass, @selector(updateUIWithContainerView:anchorView:), (IMP)DYYYProfileStoryPublishUpdateUI, &gOrigProfileStoryPublishUpdateUI, NO,
                                                "MineAvatarPlus")) {
            installed += 1;
        }
    }
    ...
    if (genericOperationClass) {
        if (DYYYInstallInstanceIMPHook(genericOperationClass, @selector(buildVirtualView:), (IMP)DYYYGenericOperationBuildVirtualView, (IMP *)&gOrigGenericOperationBuildVirtualView, YES,
                                       "MineAICreation")) {
            installed += 1;
        }
        if (DYYYInstallInstanceIMPHook(genericOperationClass, @selector(updateComponentData:), (IMP)DYYYGenericOperationUpdateComponentData, (IMP *)&gOrigGenericOperationUpdateComponentData, NO,
                                       "MineAICreation")) {
            installed += 1;
        }
    }
