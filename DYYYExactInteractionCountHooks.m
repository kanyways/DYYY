...
static BOOL DYYYExactInteractionCountsEnabled(void) {
    return [DYYYUtils fastBoolForKey:@"DYYYShowExactInteractionCounts"];
}
...
static id DYYYExactCountFormatterResult(DYYYExactCountFormatterIMP original, id self, SEL _cmd, NSNumber *number) {
...
static void DYYYInstallExactInteractionHook(const char *identifier, NSString *className, SEL selector, IMP replacement, IMP *original, const char *const *allowedEncodings,
    ...
    if (result.status != DYYYRuntimeHookInstallStatusInstalled && result.status != DYYYRuntimeHookInstallStatusAlreadyInstalled) {
        NSLog(@"[DYYY][RuntimeHook][ExactInteractionCounts] skip=%s class=%@ selector=%@ status=%@ encoding=%s", identifier, className, NSStringFromSelector(selector),
              DYYYRuntimeHookInstallStatusName(result.status), result.actualTypeEncoding ?: "(null)");
    }
}

void DYYYStartExactInteractionCountHooks(void) {
    bool expected = false;
    if (!atomic_compare_exchange_strong_explicit(&gDYYYExactInteractionCountHooksStarted, &expected, true, memory_order_acq_rel, memory_order_acquire)) {
        return;
    }

    SEL formatterSelector = @selector(showStringFromNumber:);
    DYYYInstallExactInteractionHook("interaction-counts.comment", @"AWEPlayInteractionCommentElement", formatterSelector, (IMP)DYYYCommentShowStringFromNumber, (IMP *)&gDYYYOriginalCommentShowString,
                                    kDYYYObjectNumberEncodings, sizeof(kDYYYObjectNumberEncodings) / sizeof(kDYYYObjectNumberEncodings[0]));
    DYYYInstallExactInteractionHook("interaction-counts.standard-comment", @"AWEPlayInteractionStandardCommentElement", formatterSelector, (IMP)DYYYStandardCommentShowStringFromNumber,
                                    (IMP *)&gDYYYOriginalStandardCommentShowString, kDYYYObjectNumberEncodings, sizeof(kDYYYObjectNumberEncodings) / sizeof(kDYYYObjectNumberEncodings[0]));
    DYYYInstallExactInteractionHook("interaction-counts.favorite", @"AWEPlayInteractionFavoriteElement", formatterSelector, (IMP)DYYYFavoriteShowStringFromNumber,
                                    (IMP *)&gDYYYOriginalFavoriteShowString, kDYYYObjectNumberEncodings, sizeof(kDYYYObjectNumberEncodings) / sizeof(kDYYYObjectNumberEncodings[0]));
    DYYYInstallExactInteractionHook("interaction-counts.share", @"AWEPlayInteractionShareElement", formatterSelector, (IMP)DYYYShareShowStringFromNumber, (IMP *)&gDYYYOriginalShareShowString,
                                    kDYYYObjectNumberEncodings, sizeof(kDYYYObjectNumberEncodings) / sizeof(kDYYYObjectNumberEncodings[0]));
    DYYYInstallExactInteractionHook("interaction-counts.like-update", @"AWEPlayInteractionLikeViewSubElement", @selector(updateDiggCount), (IMP)DYYYLikeUpdateDiggCount,
                                    (IMP *)&gDYYYOriginalLikeUpdateDiggCount, kDYYYVoidNoArgumentEncodings, sizeof(kDYYYVoidNoArgumentEncodings) / sizeof(kDYYYVoidNoArgumentEncodings[0]));
}
